...
public interface TestDefectHistoryRepository {
    ...
    @Transaction
    @SqlUpdate("INSERT INTO execution(defect_id, execution_id, defect_status, defect_team, defect_charge, defect_action_date, defect_action_contents) " +
            "VALUES(:defectId, :executionId, :defectStatus, :defectTeam, :defectCharge, now(), :defectActionContents) ")
    ...
    @OutParameter(name = "executionId",  sqlType = java.sql.Types.BIGINT)
    @GetGeneratedKeys
    void insertTestDefectHistory2(@BindBean TestDefectHistoryVO request);
}
